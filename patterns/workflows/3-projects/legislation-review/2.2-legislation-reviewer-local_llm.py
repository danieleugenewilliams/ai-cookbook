...
client = OpenAI(
    base_url="http://192.168.0.69:1234/v1", api_key="not-needed-for-local-llm"
)
...
def validate_legislation_content(legislation_text: str) -> LegislationValidation:
    ...
    try:
        ...
        # Return a default validation object
        return LegislationValidation(is_legislation=False, confidence_score=0.0)
...
if __name__ == "__main__":
    ...
    if not check_llm_availability():
        logger.error("Local LLM is not available. Exiting.")
        exit(1)
    ...
    # Replace the tiktoken section with a simpler approach
    try:
        ...
        logger.warning(
            f"Couldn't use tiktoken: {e}. Using character-based truncation instead."
        )
        # Fallback to simple character-based truncation
        max_chars = 16000  # Approximate - adjust based on your model's limits
